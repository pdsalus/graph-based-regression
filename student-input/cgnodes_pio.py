...
class MSELossNode(MetaNode):
    ...
    def __init__(self, y_hat: ValueNode, y: ValueNode, out: ValueNode):
        ...
        Args:
            y_hat: The predicted value node.
            y: The ground truth value node.
            out: Output node receiving the computed loss.
        """
        super().__init__()
    ...
    def forward(self):
        """Compute the 0.5 * (y_hat - y)^2 loss and push to children."""
        if self.input_ready:
    ...
    def backward(self, grad_z):
        """Compute derivatives and route gradients to parents."""
        y_hat_val, y_val = self.get_parent_values()
        # Derivative with respect to y_hat is (y_hat - y)
        self.parents[0].backward(grad_z * (y_hat_val - y_val))
        # Derivative with respect to y is -(y_hat - y)
        self.parents[1].backward(grad_z * (y_val - y_hat_val))